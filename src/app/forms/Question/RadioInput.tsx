    ...
    <>
      {options.map((el, idx) => (
        <div key={idx} className="group relative flex items-center space-x-3">
          ...
          />
          <input
            className="w-full border-b border-white bg-white py-2 hover:border-gray-300 focus:border-b-2 focus:border-orange-700"
            disabled={!hasEditAccess}
      ...
      {hasEditAccess && (
        <div className="flex items-center space-x-3">
          ...
          <button
            className="border-b border-white bg-white py-2 text-gray-400 hover:border-gray-300 focus:border-b-2 focus:border-orange-700"
            onClick={addOption}
            type="button"
          >
            Добавить вариант
