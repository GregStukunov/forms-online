...
export default function FormCard({
  ...
  async function deleteForm() {
    try {
      const confirm = window.confirm('вы уверены, что хотите удалить форму?');
  ...
  const dropdownMenu = useMemo(
    () => [
      { name: 'Открыть', onClick: () => navigate('/forms/' + id) },
      { name: 'Редактировать', onClick: () => navigate('/forms/' + id + '?edit=true') },
      { name: 'Скопировать ссылку', onClick: copyURLToClipboard },
      { name: 'Посмотреть результаты', onClick: setShowRespondents },
      { name: 'Удалить', onClick: deleteForm },
    ],
    [id, navigate],
  );
    ...
    <div className="relative space-y-2 rounded bg-white py-3 px-5 text-sm shadow">
      ...
      <div className="flex items-center space-x-1">
        ...
        <span className="text-gray-500">
          Последнее обновление {new Date(date).toLocaleString('en-US')}
        </span>
      </div>
      <Menu>
